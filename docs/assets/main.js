...
const initMask = () => {
  const inputsTel = document.querySelectorAll(".js-tel");
  const inputsMail = document.querySelectorAll(".js-mail");
  const inputsDate = document.querySelectorAll(".js-date");
  const inputsName = document.querySelectorAll(".js-name");
  ...
  inputsMail.forEach((input) => {
    const mainMask = new IMask(input, {
      mask: (value) => {
  ...
  inputsDate.forEach((input) => {
    const dateMask = new IMask(input, {
      mask: Date,
  ...
  inputsName.forEach((input) => {
    const inputMasked = new IMask(input, {
      mask: /^[A-Za-zА-Яа-я\s-]+$/
    });
    return inputMasked;
  });
...
const hasManySlides = (slider) => slider.getElementsByClassName("swiper-slide").length >= 2;
const MEDIA = {
  fullhd: [1441, 1920],
  quadhd: [1281, 1440],
  hd: [1025, 1280],
...
function clearInput(trigger) {
  const inputBox = trigger.parentElement;
  const input = inputBox == null ? void 0 : inputBox.querySelector("input");
  if (!input) {
    return;
  }
  input.value = "";
}
function addResponseText(form) {
  ...
  responseContainer == null ? void 0 : responseContainer.appendChild(paragraph);
  form.classList.add("is-submitted");
}
function initFormsNew() {
  const inputClearBtns = document.querySelectorAll(".base-input__clear");
  inputClearBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      clearInput(btn);
    });
  });
  const forms = document.querySelectorAll("[data-form]:not(.is-init)");
  forms.forEach((form) => {
    form.addEventListener("submit", function(event) {
      ...
      elements.forEach((input) => {
        var _a;
        (_a = input == null ? void 0 : input.parentElement) == null ? void 0 : _a.classList.remove("is-error");
        input.addEventListener("input", function() {
          var _a2;
          if (input.value.trim()) {
            (_a2 = input == null ? void 0 : input.parentElement) == null ? void 0 : _a2.classList.remove("is-error");
          }
        });
      });
      let isValid = true;
      elements.forEach((input) => {
        ...
        if (input.type === "checkbox") {
          if (!input.checked) {
            (_a = input == null ? void 0 : input.parentElement) == null ? void 0 : _a.classList.add("is-error");
            isValid = false;
          ...
          if (inputValue === "") {
            isValid = false;
            (_b = input == null ? void 0 : input.parentElement) == null ? void 0 : _b.classList.add("is-error");
          } else {
            isValid = true;
          }
        }
